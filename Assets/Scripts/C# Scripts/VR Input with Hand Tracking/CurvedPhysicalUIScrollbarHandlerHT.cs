...
/// <summary>
/// [ID] Menangani interaksi VR Scrollbar dengan Hand Tracking (Auto-Hover Drag).
...
/// 
/// [EN] Handles VR Scrollbar interaction with Hand Tracking (Auto-Hover Drag).
...
/// </summary>
public class CurvedPhysicalUIScrollbarHandlerHT : MonoBehaviour
{
    ...
    // [ID] Tidak lagi menggunakan Input Action Pinch karena kita murni mengandalkan Hover (Sentuhan Raycast)
    // [EN] No longer using Pinch Input Actions because we rely purely on Hover (Raycast Hit)

    private void Start()
    {
        if (scrollbarColliders.Length != canvasScrollbars.Length)
            Debug.LogError($"[PhysicalScrollbarHT] Mismatch! Colliders: {scrollbarColliders.Length}, Scrollbars: {canvasScrollbars.Length}.");
    }
    ...
    private void Update()
    {
        ...
        // [ID] Cek tangan Kanan (Bisa menimpa input kiri jika mengenai scrollbar lain)
        // [EN] Check Right hand (Can override left input if it hits another scrollbar)
    ...
    {
        // [ID] Cari apakah objek yang tersorot laser adalah salah satu collider scrollbar kita
        // [EN] Check if the object hit by the laser is one of our scrollbar colliders
        ...
        {
            UpdateScrollbarValue(index, hit.point, interactorName);
        }
    }

    private void UpdateScrollbarValue(int index, Vector3 worldHitPoint, string interactorName)
    {
        ...
        if (col != null && uiScrollbar != null)
        {
            ...
            if (showDebug)
                Debug.Log($"[ScrollbarHT] Val: {normalizedValue:F2} via {interactorName} (Auto-Hover)");
        }
    }
