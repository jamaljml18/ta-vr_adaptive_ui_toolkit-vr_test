# CurvedPhysicalUIScrollviewHandlerHT

## 🇬🇧 English Documentation

### Overview

`CurvedPhysicalUIScrollviewHandlerHT` is a script designed to handle VR interactions for Canvas UI elements—specifically curved interfaces—using a **"Physical Proxy"** approach for **Scroll Views / Scrollbars**, completely optimized for **Hand Tracking (Auto-Hover)**.

A physics-based interaction system maps BoxCollider proxies to UI elements. This ensures precise raycast detection regardless of the visual mesh curvature or texture resolution. When dealing with long lists or scroll views in VR, requiring the user to hold a pinch gesture and drag their hand can lead to fatigue or loss of tracking. This script uses a seamless **Auto-Hover** architecture. You do not need to pinch or click. By simply pointing your hand laser at the invisible 3D Box Collider (acting as the scroll track) and moving your hand along it, the script calculates your laser's position on the local X-axis in real-time. It instantly updates the UI Scrollbar (0.0 to 1.0), scrolling the content effortlessly with just a hand motion.

### Features

- **Pinch-Free Auto-Hover Interaction:** The scroll view updates continuously the moment the VR laser touches the collider. No holding, grabbing, or pinching required.
...
3. **Attach Script:** Attach the `CurvedPhysicalUIScrollviewHandlerHT` script to a manager object (e.g., the Canvas itself).
...
> **Note:** Ensure the Collider's `Size X` matches the visual length of the UI scrollbar track. The script uses this Size X to map the 0 to 1 value.
...
`CurvedPhysicalUIScrollviewHandlerHT` adalah skrip yang dirancang untuk menangani interaksi VR pada elemen Canvas UI—khususnya antarmuka yang melengkung (curved)—menggunakan pendekatan **"Physical Proxy"** khusus untuk **Scrollbar** (biasanya digunakan pada Scroll View), dan dioptimalkan sepenuhnya untuk **Hand Tracking (Auto-Hover)**.

Sistem interaksi berbasis fisika ini memetakan proxy BoxCollider ke elemen UI. Hal ini memastikan deteksi raycast yang presisi tanpa mempedulikan kelengkungan mesh visual atau resolusi tekstur. Saat berhadapan dengan daftar panjang atau menu gulir di VR, mengharuskan pengguna menahan gestur cubitan (pinch) sambil menggerakkan tangan bisa memicu kelelahan atau hilangnya pelacakan sensor. Skrip ini menggunakan arsitektur **Auto-Hover** yang mulus. Anda tidak perlu mencubit atau mengklik. Hanya dengan mengarahkan laser dari tangan kosong Anda ke Box Collider 3D dan menggerakkan tangan di sepanjang jalurnya, skrip akan menghitung posisi laser pada sumbu X lokal secara real-time dan langsung memperbarui Scrollbar UI (0.0 hingga 1.0).
...
3. **Pasang Skrip:** Pasangkan skrip `CurvedPhysicalUIScrollviewHandlerHT` pada objek manajer (misalnya, di Canvas itu sendiri).
4. **Petakan Array:**
...
| **Atas ke Bawah**          | `(0, 0, -90)`    | Vertikal Terbalik (Paling umum untuk membaca atas ke bawah) |
...
3. Saat pemain mengarahkan laser VR mereka ke ujung atas `Collider_JalurInventaris`, inventaris langsung melompat ke paling atas. Saat mereka cukup menggerakkan tangan ke bawah (tanpa perlu mencubit), isi menu Scroll View akan bergulir ke bawah secara mulus dan real-time.
